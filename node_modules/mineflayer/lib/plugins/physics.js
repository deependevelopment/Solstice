...
function inject (bot, { physicsEnabled, maxCatchupTicks }) {
  ...
  function tickPhysics (now) {
    ...
    if (bot.physicsEnabled && shouldUsePhysics) {
      ...
      bot.emit('physicsTick') // Deprecated, only exists to support old plugins. May be removed in the future
    }
    if (shouldUsePhysics) {
      updatePosition(now)
    }
  ...
  // remove this when 'physicsTick' is removed
  bot.on('newListener', (name) => {
    if (name === 'physicsTick') console.warn('Mineflayer detected that you are using a deprecated event (physicsTick)! Please use this event (physicsTick) instead.')
  })
